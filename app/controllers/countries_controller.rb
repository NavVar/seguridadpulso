class CountriesController < ApplicationController
  ...
  def index
    if params[:search]
      @countries= Country.search(params[:search]).order("code").paginate(:page => params[:page], :per_page => 8)
    else
      @countries = Country.all.order("code").paginate(:page => params[:page], :per_page => 8)
    end
  end
